...
public class MyActionListener implements ActionListener{
    ...
    */
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        else if(e.getSource()==Screen.getMy_BUTTONSpanel().getFINDbutton()){  //ENCONTRAR

            Screen.getMy_CENTERpanel().getPanel().setVisible(false);
            Screen.getMy_CENTERpanel().getPanel().removeAll();
            Screen.getMy_CENTERpanel().getPanel().add(Screen.getMy_CENTERpanel().getMyFindPanel().getPanel());
            Screen.getMy_CENTERpanel().getPanel().setVisible(true);

        }
        else if(e.getSource()==Screen.getMy_CENTERpanel().getMyFindPanel().find_btn){ //OK ENCONTRAR

            FindPanel findPanel = Screen.getMy_CENTERpanel().getMyFindPanel();

            //confere se o id é um número inteiro (se não for, aparece uma mensagem explicando o erro)

            //confere se o nome da coluna existe na tabela (só pode ser "nome","preco","marca","validade","quantidade" ou "setor") (se não existir, aparece uma mensagem explicando o erro)
            
            //caso não haja nenhum desses dois erros, chama a função:
            //Queries.returnSpecificValue(id, column)

        }
        else if(e.getSource()==Screen.getMy_BUTTONSpanel().getCLEARbutton()){ //ESVAZIAR

            Screen.getMy_CENTERpanel().getPanel().setVisible(false);
            Screen.getMy_CENTERpanel().getPanel().removeAll();
            Screen.getMy_CENTERpanel().getPanel().add(Screen.getMy_CENTERpanel().getMyClearPanel().getPanel());
            Screen.getMy_CENTERpanel().getPanel().setVisible(true);

        }
        else if(e.getSource()==Screen.getMy_CENTERpanel().getMyClearPanel().clear_btn){ //OK ESVAZIAR
            
            //mostrar uma mensagem de alerta com JOptionpanel perguntando se o usuário tem certeza que quer apagar tudo
            //caso ele selecione que sim:
            Queries.clearTable();
